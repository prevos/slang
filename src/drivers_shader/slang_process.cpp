   ...
   for (semantic = 0; semantic < SLANG_NUM_TEXTURE_SEMANTICS; semantic++)
   {
            ...
            sl_reflection.semantic_textures[semantic].size(); index++)
      {
         ...
         if (src.stage_mask)
         {
            ...
            {
               texture.wrap    = static_cast<slang_wrap_type_t>(shader_info->lut[index].wrap);
               texture.filter  = static_cast<slang_filter_t>(shader_info->lut[index].filter);
            }
            else
            {
               texture.wrap    = static_cast<slang_wrap_type_t>(shader_info->pass[pass_number].wrap);
               texture.filter  = static_cast<slang_filter_t>(shader_info->pass[pass_number].filter);
            }
      ...
      pass_semantics_t*      out)
{
   ...
   Compiler*          vs_compiler = nullptr;
   Compiler*          ps_compiler = nullptr;
   ...
   pass.source.string.vertex   = nullptr;
   pass.source.string.fragment = nullptr;
   ...
   try
   {
      ...
      if (dst_type == RARCH_SHADER_METAL)
      {
         ...
         auto vs = (CompilerMSL*)vs_compiler;
         auto ps = (CompilerMSL*)ps_compiler;
